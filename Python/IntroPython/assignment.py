...
STUDENT_NAME = "Ewen Gueguen"
STUDENT_ID = "261235236"
...
def q6(x):
    ...
    """
    return sum(sum((2 * i + j) for j in range(0, 6)) for i in range(0, x+1))
...
def q7(x):
    ...
    * Values at even indices must be positive, and values at odd indices must be negative.
    ...
    """
    return [i * 3 * (1 if i % 2 == 0 else -1) for i in range(x)]
...
def q11(x, y, z):
    ...
    answer: [0, 1, 0]
    """
    return (lambda n=math.sqrt(x**2 + y**2 + z**2): [0.0, 0.0, 0.0] if n == 0 else [x / n, y / n, z / n])()
...
def q12(radius, degrees):
    ...
    """
    return math.cos(math.radians(degrees)) * radius, math.sin(math.radians(degrees)) * radius
...
def q13(data: list[list[int]]):
    ...
    return "".join(f"{min(i)},{max(i)},{mean(i)},{median(i)}\n" for i in data)
...
def q14(text: str):
    ...
    * The numbers will all be integers within -2147483648 to 2147483647.
    """
    return [[int(c) for c in line.split(",")] for line in text.split("\n")]
